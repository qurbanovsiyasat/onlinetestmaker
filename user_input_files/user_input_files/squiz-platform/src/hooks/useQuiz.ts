...
export function useQuiz(quizId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return { ...quiz, questions: parsedQuestions as Question[] }
    },
    enabled: !!quizId,
  })
}
...
export function useQuizLeaderboard(quizId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      // Get first attempt for each user using window functions
      const { data, error } = await supabase
      ...
      if (error) {
        ...
        fallbackData?.forEach(result => {
          ...
          if (!userFirstAttempts.has(userId) || 
              new Date(result.created_at) < new Date(userFirstAttempts.get(userId).created_at)) {
