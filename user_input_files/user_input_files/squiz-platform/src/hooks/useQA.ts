...
// Get all Q&A questions with enhanced search and filtering
export function useQAQuestions(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Convert category string to UUID if needed
      let categoryUuid = null
      if (category && category !== 'all') {
        // If category is already a UUID, use it directly
        if (category.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
          categoryUuid = category
        } else {
          // If it's a category name, find the UUID
          const { data: categoryData } = await supabase
            ...
            .eq('type', 'qa')
            .single()
          categoryUuid = categoryData?.id || null
        }
      }
...
export function useVoteQAQuestion() {
  ...
  return useMutation({
    mutationFn: async ({ questionId, voteType }: { questionId: string; voteType: 'up' | 'down' }) => {
      ...
      // First, let's try to update the vote score directly in the database
      ...
      // Calculate new score
      const currentScore = question.votes_score || 0
...
export function useVoteQAAnswer() {
  ...
  return useMutation({
    mutationFn: async ({ answerId, voteType, questionId }: { answerId: string; voteType: 'up' | 'down'; questionId: string }) => {
      ...
      // Calculate new score
      const currentScore = answer.votes_score || 0
      const increment = voteType === 'up' ? 1 : -1
      const newScore = Math.max(0, currentScore + increment)
      
      // Update the answer's vote score
      const { error: updateError } = await supabase
        .from('qa_answers')
        .update({ votes_score: newScore })
        .eq('id', answerId)
...
export function useIncrementQuestionView() {
  return useMutation({
    mutationFn: async (questionId: string) => {
      ...
      const newViewCount = (question.views || 0) + 1
