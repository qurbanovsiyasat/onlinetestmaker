...
export default function FormPage() {
  ...
  const renderFormField = (field: any) => {
    ...
    switch (field.field_type) {
      ...
      case 'number':
        return (
          <div key={field.id} className="mobile-form-field space-y-2">
            <Label htmlFor={fieldId} className="typography-body">
              {field.label}
            ...
            <div className="space-y-3">
              {field.options?.map((option: string, index: number) => (
                <div key={index} className="flex items-start space-x-3 p-3 rounded-lg border hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  ...
                  />
                  <Label htmlFor={`${fieldId}-${index}`} className="typography-body font-normal text-break flex-1">
                    {option}
                  </Label>
                </div>
              ))}
  ...
  return (
    <div className="mobile-form-container space-y-6 py-6">
      {/* Form Header */}
      <motion.div
          ...
          <CardHeader>
            <div className="flex flex-col space-y-4 md:flex-row md:space-y-0 md:items-start md:justify-between">
              <div className="flex-1 min-w-0">
                <CardTitle className="typography-h2 mb-2 text-break">{form.title}</CardTitle>
                {form.description && (
                  <CardDescription className="typography-body mb-4 text-break">
                    {form.description}
                  </CardDescription>
                )}
                
                <div className="flex flex-col space-y-2 md:flex-row md:space-y-0 md:space-x-4 typography-small text-slate-600 dark:text-slate-400">
                  <div className="flex items-center space-x-1">
                    <User className="h-4 w-4" />
                    <span className="text-break">{form.creator?.full_name || 'Anonymous'}</span>
                  </div>
                  <div className="flex items-center space-x-1">
