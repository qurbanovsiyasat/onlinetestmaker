...
import { ArrowLeft, Save, Loader2, Plus, X, HelpCircle, Image } from 'lucide-react'
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.category} onValueChange={(value) => handleInputChange('category', value)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Kateqoriya seçin (məcburuiri deyil)" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-3">
                ...
                <div className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center hover:border-slate-400 transition-colors">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (files.length > 0) {
                        toast.success(`${files.length} şəkil seçildi`)
                      }
                    }}
                    className="hidden"
                    id="image-upload"
                  ...
                  <label htmlFor="image-upload" className="cursor-pointer">
                    ...
                    <p className="text-xs text-slate-500">
                      PNG, JPG və ya GIF formatında olmalıdır
                    </p>
                  </label>
                </div>
                {images.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium">Seçilmiş şəkillər:</p>
                    <div className="flex flex-wrap gap-2">
                      {images.map((file, index) => (
                        <div key={index} className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded-full">
                ...
                <p className="text-xs text-slate-500">
                  Maksimum 3 şəkil əlavə edə bilərsiniz. Şəkillər sualınızı daha aydın etmək üçün istifadə edin.
                </p>
              </div>
